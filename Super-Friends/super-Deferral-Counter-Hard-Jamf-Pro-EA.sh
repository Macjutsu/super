...
# This script returns the number of times the Hard Deadline had been deferred by super 2.0 or later.
...
if [[ -f "$superPLIST.plist" ]]; then
	superCOUNTER=$(defaults read "$superPLIST" HardCounter)
	# Report if the file has a value.
	if [[ -n $superCOUNTER ]]; then
		echo "<result>$superCOUNTER</result>"
	else
		echo "<result>No Hard Deadline deferrals.</result>"
	fi
else
