#!/bin/bash
# This script generates a Jamf Pro API post call to start the MDM update workflow via the LATEST_MINOR or LATEST_MAJOR versions of macOS.
# The Jamf Pro API calls made by this script are identical to those in super v4.0.x.
...
#
# TESTING macOS UPDATE workflows:
# 1. Fill in the CAPITALIZED parameter values in the script below.
# 2. Make sure the "UPGRADE_WORKFLOW" parameter in the script below is set to "FALSE".
...
#
# TESTING macOS UPGRADE workflows:
# 1. Fill in the CAPITALIZED parameter values in the script below.
# 2. Make sure the "UPGRADE_WORKFLOW" parameter in the script below is set to "TRUE".
# 3. If you want to test BETA macOS updates or upgrades make sure to enroll the system into a beta seed before continuing to step 4.
# 4A. Pre-download macOS UPGRADE for macOS 13.x -> macOS 14.x:
#	- On the test client in Terminal use: sudo softwareupdate --list.
#	- On the test client in Terminal copy the * Label: name of the update target.
#	- On the test client in Terminal use: sudo softwareupdate --download "Label Name"; for example: sudo softwareupdate --download "macOS Sonoma 14.1.1-23B81"
#	- Wait for the macOS download to complete...
...
if [[ -z "${UPGRADE_WORKFLOW}" ]] || { [[ "${UPGRADE_WORKFLOW}" != "TRUE" ]] && [[ "${UPGRADE_WORKFLOW}" != "FALSE" ]]; }; then 
	echo "Error: You must populate the UPGRADE_WORKFLOW parameter with either TRUE or FALSE."; exit_error="TRUE"
fi
...
[[ "${UPGRADE_WORKFLOW}" == "FALSE" ]] && macos_major_upgrade_version_major_target="FALSE"
...
# This function calls similar Jamf API commands to that of super.
create_mdm_command() {
...
# Create an MDM update command via the Jamf Pro API. This code is identical that found in super.
