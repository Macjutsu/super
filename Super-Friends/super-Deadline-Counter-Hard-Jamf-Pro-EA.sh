...
# This script returns the number of times the user has deferred the Deadline Hard via super 2.0 or later.
...
if [[ -f "${SUPER_LOCAL_PLIST}.plist" ]]; then
	...
	if [[ $(echo "${super_version_local}" | cut -c 1) -ge 4 ]]; then
		super_count=$(defaults read "${SUPER_LOCAL_PLIST}" DeadlineCountHard 2> /dev/null)
		super_counter=$(defaults read "${SUPER_LOCAL_PLIST}" DeadlineCounterHard 2> /dev/null)
	else # super version 3 or older.
		super_count=$(defaults read "${SUPER_LOCAL_PLIST}" HardCount 2> /dev/null)
		super_counter=$(defaults read "${SUPER_LOCAL_PLIST}" HardCounter 2> /dev/null)
	fi
	...
	if [[ -n "${super_counter}" ]]; then
		if [[ $((super_counter + 1)) -le ${super_count} ]]; then
			...
			echo "<result>Deadline Count Hard of ${super_count} has passed.</result>"
		fi
	else
		[[ -n "${super_count}" ]] && echo "<result>0</result>"
		[[ -z "${super_count}" ]] && echo "<result>Deadline Count Hard is not enabled.</result>"
	fi
else
