#!/bin/bash
# This script generates unfiltered ManagedClient and softwareupdated logs on a system that is being pushed the MDM macOS update/upgrade workflows.
# The logs are saved to the current user's Desktop and automatically opened in the Console.app.

# Name for the ManagedClient log:
mdmCommandLOG="ManagedClient.log"

# Name for the softwareupdated log:
mdmUpdateLOG="softwareupdated.log"
...
rm -f "$mdmCommandLOG"
log stream --style compact --predicate 'subsystem == "com.apple.ManagedClient"' >> "$mdmCommandLOG" &
mdmCommandStreamPID=$!
...
rm -f "$mdmUpdateLOG"
log stream --style compact --predicate 'process == "softwareupdated"' >> "$mdmUpdateLOG" &
